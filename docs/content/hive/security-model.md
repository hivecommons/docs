> **Synced from Hive.** This page is pulled from [hivecommons/hive@v5](https://github.com/hivecommons/hive/blob/v5/src/docs/security-model.md) during the docs build. Edit the canonical source in the Hive repository.
...
- **Proposed gate-integrity invariants.** The [gate-integrity design](https://github.com/hivecommons/hive/blob/v5/src/docs/design/gate-integrity-invariants.md) proposes that the GitHub App write gate / push proxy refuse agent history rewrites on branches the lane did not create, refuse agent sign-off trailers added to other authors' commits, and treat gate manipulation as an ACMM regression that demotes the lane to hold-gated pending human review.
...
The hub master secret supports **generations**: at most two live at once — one CURRENT (mints new material) and one PREVIOUS (verify-only, default window 7 days). Design details: [design/master-key-rotation.md](https://github.com/hivecommons/hive/blob/v5/src/docs/design/master-key-rotation.md).
