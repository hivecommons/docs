> **Synced from Hive.** This page is pulled from [hivecommons/hive@v4](https://github.com/hivecommons/hive/blob/v4/src/docs/security-model.md) during the docs build. Edit the canonical source in the Hive repository.
...
The hub master secret supports **generations**: at most two live at once — one CURRENT (mints new material) and one PREVIOUS (verify-only, default window 7 days). Design details: [design/master-key-rotation.md](https://github.com/hivecommons/hive/blob/v4/src/docs/design/master-key-rotation.md).
