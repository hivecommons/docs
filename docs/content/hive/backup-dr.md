...
- a daily `CronJob` scheduled at `17 3 * * *`, `concurrencyPolicy: Forbid`, one-hour active deadline, running `ghcr.io/hivecommons/hive:stable` with `imagePullPolicy: Always` (the backup image tracks the stable [release channel](/docs/hive/release-channels), [#3810](https://github.com/hivecommons/hive/pull/3810));
...
| `systemctl --user start hive-data-volume.service` | the four `io.hivecommons.hive.*` labels | yes — "volumes (1): hive-data" |
| `bin/hive-podman-setup.sh`, volume unit stale-`active` | the four `io.hivecommons.hive.*` labels — the installer restarts the unit when the volume is missing (#4485) | yes |
