> **Synced from Hive.** This page is pulled from [hivecommons/hive@v5](https://github.com/hivecommons/hive/blob/v5/src/docs/work-sources.md) during the docs build. Edit the canonical source in the Hive repository.

# Work sources (`governor.work_source`)

`work_source` selects where a hive reads actionable work items from (Step 01
of the governor loop — see [Architecture](/docs/hive/architecture)). It accepts four
...
[Linear agent integration](https://github.com/hivecommons/hive/blob/v5/src/docs/linear-agent.md) — for the two-way agent-session
...
See [Linear agent integration](https://github.com/hivecommons/hive/blob/v5/src/docs/linear-agent.md) for the OAuth app setup,
...
team) — see [ACMM policy matrix → Where gap issues are filed](/docs/hive/acmm-policy-matrix#where-acmm-gap-issues-are-filed).
