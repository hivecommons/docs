...
describe('getBaseUrl — client-side hostname validation', () => {
  ...
  it('does NOT match evil-previews.hivecommons.dev (prefix attack)', async () => {
    mockWindow('evil-previews.hivecommons.dev', 'https:')
    const getBaseUrl = await loadGetBaseUrl()
    // Should fall through to production URL, NOT return current host
    expect(getBaseUrl()).not.toBe('https://evil-previews.hivecommons.dev')
  })
  ...
  it('returns production URL for hivecommons.dev', async () => {
    mockWindow('hivecommons.dev', 'https:')
    const getBaseUrl = await loadGetBaseUrl()
    expect(getBaseUrl()).toBe('https://docs.hivecommons.dev')
  })
})
