...
function detectCurrentVersionKey(versions: Array<{ key: string; branch: string; isDev?: boolean; isDefault?: boolean }>): string {
  ...
  if (hostname === 'docs.hivecommons.dev') {
...
export function VersionSelector({ className = '', isMobile = false }: VersionSelectorProps) {
  ...
  const handleVersionChange = (versionKey: string) => {
    ...
    if (versionKey === 'latest' || version.isDefault) {
      window.location.href = `https://docs.hivecommons.dev${pathname}`;
      return;
    }

    // Other versions use Netlify branch deploys
    // Netlify converts branch names: docs/0.5.0 -> docs-0-5-0
    const branchSlug = version.branch.replace(/\//g, '-').replace(/\./g, '-');
    window.location.href = `https://${branchSlug}--hivecommons-docs.netlify.app${pathname}`;
  };
