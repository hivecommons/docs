...
const PRODUCTION_URL = 'https://docs.hivecommons.dev';
...
export function RelatedProjects({ variant = 'full', onCollapse, bannerActive = false, legacyPageMap, autoExpandLegacy = false, generalSections = [] }: RelatedProjectsProps) {
  ...
  useEffect(() => {
    ...
    // Use relative URLs on: localhost, hivecommons.dev, and preview/staging deploys (netlify)
    // Only use absolute URLs if explicitly on a different domain
    const checkProduction =
      window.location.hostname === 'localhost' ||
      window.location.hostname === 'docs.hivecommons.dev' ||
      window.location.hostname === 'www.docs.hivecommons.dev' ||
  ...
  const getCurrentProject = () => {
    if (pathname.startsWith('/docs/hotshot')) return 'hotshot';
